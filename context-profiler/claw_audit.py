...
class ContextAuditor:
    ...
    def check_redundancy(self) -> List[TurnStats]:
        ...
        # Check if consecutive tool outputs have errors OR if errors repeat with a model turn in between
        for i in range(len(self.turns) - 2):
            t1 = self.turns[i]
            t3 = self.turns[i+2] # Check skip-1 neighbor (Tool -> Model -> Tool)
            
            if t1.has_error and t3.has_error and t1.role == "function" and t3.role == "function":
                 t3.is_redundant = True
                 t3.redundancy_reason = "Oscillation: Repeated Errors (Interleaved)"
                 redundant_turns.append(t3)
                 
        # Also check immediate neighbors (just in case)
        for i in range(len(self.turns) - 1):
            t1 = self.turns[i]
            t2 = self.turns[i+1]
            if t1.has_error and t2.has_error and t1.role == t2.role:
                 t2.is_redundant = True
                 t2.redundancy_reason = "Oscillation: Repeated Errors (Immediate)"
    ...
    def print_report(self):
        ...
        analysis_layout = Layout(name="analysis", ratio=1)
        layout["main"].split_row(
            Layout(Panel(table), ratio=2),
            analysis_layout
        )
        analysis_layout.split_column(issue_panel, timeline_panel)
