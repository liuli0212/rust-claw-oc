...
- Hybrid Mode
  ...
  - for example: use a direct tool to inspect state, then `exec` for batch work, then a direct final visible text response
...
- nested completion is not allowed in early phases
- final visible text response remains the top-level lifecycle signal
- if `exec` determines the task is complete, it should return that conclusion to the model, and the model may then emit a top-level final visible text response
...
- nested completion is disallowed
...
- top-level final visible text response still ends run after code mode output indicates completion
